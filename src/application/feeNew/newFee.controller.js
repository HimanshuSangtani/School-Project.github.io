app.controller('newFeeController', function ($rootScope, $location, HandShakeService, FlashService, $scope, $log, UtilService, HttpService, $indexedDB, $sessionStorage,$timeout, $firebaseAuth, Auth) {
    ...
    $scope.cat=function(){
        ...
        $rootScope.mypageloading = true;
        var requestHandle = HttpService.HttpGetData($rootScope.serverURL + '/customer/v2/feecat');
            requestHandle.then(function (result) {
                $rootScope.mypageloading = false;
            if (result.success)
            {   
                if(result.data.feecatList)
                $scope.Category = result.data.feecatList;
                else $scope.Category = result.data;
    ...
    $scope.save = function (){
        ...
        $rootScope.mypageloading = true;
        var requestHandle = HttpService.HttpPostData($rootScope.serverURL + '/customer/v2/feecat', $scope.AddCat);
            requestHandle.then(function (result) {
                $rootScope.mypageloading = false;
    ...
    $scope.openFeeDetails = function (fee) {
            $sessionStorage.mode = '';
            $sessionStorage.className = fee.name;
            $location.path('/newFee/:' + fee._id);
    }
