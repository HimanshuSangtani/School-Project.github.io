...
app.controller('ClassAttendanceController', function ($rootScope, $scope, UtilService,  HttpService, HandShakeService, mapService, $sessionStorage, $timeout, $log) {
    $log.debug("ClassAttendanceController reporting for duty.");
    ...
    $scope.showStuList = function ($index) {
        $scope.classId = $scope.classList[$index]._id;
    ...
    $scope.getHistoryReport = function (fromDate, toDate) {
        ...
        if ($scope.fromDate && $scope.toDate)
        {
            ...
            $scope.student_list = [];
            dayHash = [];
            var requestHandle = HttpService.HttpGetData($rootScope.serverURL + '/customer/teacher/attendance?class_id=' + $scope.classId + '&time=' + 1554686065000);
            requestHandle.then(function (result) {
                $rootScope.mypageloading = false;
                if (result.success == true)
                {
                    $scope.attendanceListOfMonth = result.data;


                    for (var attDay in attendanceListOfMonth) {
                        studAttendanceList.push(studAttendance);
                      }
